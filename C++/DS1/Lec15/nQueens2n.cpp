// Code- Proactive code  of 2 raise to the power n for nQueens
#include<iostream>
#include<vector>
#include<cstring>
using namespace std;
...
void nQueens(vector<vector<bool>> & box, int qpsf, int cb, string asf){
    if(cb == box.size() * box.size()){
        if(qpsf == box.size()){
        cout << asf << endl; 
        }
        return;
    ...
    nQueens(box, qpsf, cb + 1, asf);
    //Trick - conversion of 1D array into 2D and  2D array into 1D makes traversal easier
    int i = cb / box.size();
    int j = cb % box.size();
    if(box[i][j] == false && isQueenSafe(box, i, j)){
        box[i][j] = true;
        nQueens(box, qpsf + 1, cb + 1, asf + to_string(i) + to_string(j) + " ");
        box[i][j] = false;
    }
