...
public:
    ...
    int dls;
    int profit;
    JHelper(char job, int dls, int profit)
    {
        this->job = job;
        this->dls = dls;
        this->profit = profit;
    ...
    bool operator<(const JHelper &other) const
    {
        return this->profit < other.profit;
    }
};

void jobSequencing(string jobs, vector<int> &dls, vector<int> &profit)
{
    priority_queue<JHelper> pq;
    int dm = INT_MIN;
    for (int i = 0; i < dls.size(); i++)
    {
        JHelper j(jobs[i], dls[i], profit[i]);
        pq.push(j);
        if (dls[i] > dm)
        {
            dm = dls[i];
    ...
    vector<char> ans(dm + 1);

    while (pq.size() > 0)
    {
        JHelper tj = pq.top();
        pq.pop();
        int dlsl = find(dsp, tj.dls);
        if (dlsl > 0)
        {
            ans[dlsl] = tj.job;

            // merging
            dsp[dlsl] = find(dsp, dlsl - 1);
        }
    }
...
int mytime = 0;
void APsandBridges(vector<vector<Edge *>> &graph, vector<int> &parent, vector<int> &disc,
                   vector<int> &low, vector<bool> &visited, vector<bool> &AP, int src)
{
    visited[src] = true;
    disc[src] = low[src] = mytime;
    mytime++;
    int counter = 0; // for detection of root
    ...
    for (int e = 0; e < graph[src].size(); e++)
    {
        ...
        {
            parent[nbr] = src;
            APsandBridges(graph, parent, disc, low, visited, AP, nbr);
            ...
            if (low[nbr] > disc[src])
            {
                cout << nbr << " and " << src << " are Bridges." << endl;
            }
        }
    }
}
...
*/
    ...
    vector<int> profits{100, 67, 94, 44, 33, 77, 11, 99, 37, 76, 34, 47, 78, 44, 39, 75, 49, 33, 48, 92, 43, 90};
    jobSequencing(names, deadlines, profits);

    vector<int> parent(graph.size());
    parent[0] = -1;
    vector<int> disc(graph.size());
    vector<int> low(graph.size());
    vector<bool> AP(graph.size(), false);
    vector<bool> visited(graph.size(), false);
    APsandBridges(graph, parent, disc, low, visited, AP, 0);
    for (int i = 0; i < AP.size(); i++)
    {
        if (AP[i] == true)
        {
            cout << i << " is and Articulation Point." << endl;
        }
    }
}